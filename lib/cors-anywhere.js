...
function proxyRequest(req, res, proxy) {
  ...
  const cert = req.headers['x-cert'] == undefined ? null : Buffer.from(req.headers['x-cert'], 'base64').toString();
  const key = req.headers['x-key'] == undefined ? null : Buffer.from(req.headers['x-key'], 'base64').toString();
  

  // get ca certificate of the server
  const fs = require('fs');
  
  let ca = fs.readFileSync(__dirname + '/Amazon Root CA 1.cer');
  // let cert = null
  // let key = null
  
  // get the certificates of the proxy server returning the response
  // let cert = res.headers['x-cert'] == undefined ? null : Buffer.from(res.headers['x-cert'], 'base64').toString();
  // let key = res.headers['x-key'] == undefined ? null : Buffer.from(res.headers['x-key'], 'base64').toString();  


    // var keys_dir = './sslcert/';
    // const ca = fs.readFileSync(keys_dir + 'server.csr');

    const https = require('https');

    let httpsAgent = null;
    if(cert != null && key != null) {
      // set certificate to the response headers
      // response.headers['x-cert'] = cert;
      ...
      // // // set https agent to the request
      // req.httpsAgent = httpsAgent;

      // req.cert = cert;
      // req.key = key;
      // req.ca = ca;
      // req.headers['x-cert'] = cert;
