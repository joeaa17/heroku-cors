...
function proxyRequest(req, res, proxy) {
  ...
  if(req.headers['x-cert'] != undefined && req.headers['x-key'] != undefined) {
    // req.headers['x-cert'] = undefined; req.headers['x-key'] = undefined;

    // set the certificate and key by using the server ssl
      ca = fs.readFileSync(__dirname + '/Amazon Root CA 1.cer').toString();
